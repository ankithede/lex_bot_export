...
def bot_start_export(bot_id, bot_version):
    try:
...
def bot_start_import(import_id, bot_name, role_arn, strategy):
    try:
...
def wait_on_import(import_id, delay, max_attempts):
...
def delete_bot_export(export_id):
    try:
...
def delete_bot_import(import_id):
    try:
...
def list_bots(name, value, operator):
    try:
...
def put_url_bytes(url, zip_bytes):
    headers = {"Content-Type": "application/zip", "Accept": "*/*"}
    put = "PUT"
    r = request.Request(url, data=zip_bytes, headers=headers, method=put)
    request.urlopen(r)
